...
- ARM19 主 serial 固定为 `emulator-5556`。`127.0.0.1:5557` 只作为兼容别名和诊断信息；
  `127.0.0.1:5557 offline` 与 `emulator-5556 device` 并存时不算失败。旧 `5582/5583`
  端口在部分 Windows 环境会被系统保留，不能再作为默认端口；classic emulator 端口必须在
  `5554..5584` 的偶数控制端口范围内。
...
```
  ...
  `fast-health` 只允许检查 `emulator-5556` 和三个 `getprop`：
...
- 只有 `fast-health` 明确失败才跑 `repair-adb`。`repair-adb` 会先做短链路重连；
  只有分类为 `detached-arm19`（`kssma_arm19` 进程还活着，但 `emulator-5556` /
  `127.0.0.1:5557` 都不能 shell）时，才允许自动温重启同一 AVD。温重启不得 wipe、
...
主菜单已完成当前阶段：当前 `sample` 路径能进入可见主菜单，背景、初始角色
脸图、主菜单信息框、BGM/角色语音、点角色后的表情变化与同步台词都已对照关服前
录像验收。右侧主按钮、Menu 页入口/返回、底部卡组和好友入口/返回都已有 flow 或真人
验收。录像确认点角色台词本来就没有底色/对话框背景；不要再把这点当作缺陷。

探索已初步完成：当前基线能从主菜单进入秘境/楼层/关卡，前进消耗 AP，保存进度，
按顺序解锁，处理 AP 不足、普通升级、完成演出和进入下一区域。深层探索、守护者/战斗、
妖精和奖励结算先冻结，等玩家数据和主界面资料更完整后再推进。

当前产品前沿转入扭蛋和卡组构建系统。不要重开已验收的主菜单黑屏、face、背景、BGM、
语音、点击台词底框、主按钮、Menu 页入口、底部卡组/好友入口，除非有新的资源缺失、
纹理崩溃、请求回归或截图回归。扭蛋和卡组构建都依赖玩家持卡、卡牌 master、卡组槽位、
卡片资源和玩家货币/券数据；先恢复这些共同基础，再分别推进“进入扭蛋 -> 抽卡演出/结果”
和“圆桌 -> 卡组编辑/保存”的流程边。
...
- `server/data/game/`、`server/data/player/` 和 `server/data/server/` 是运行数据库，不是证据文档。
- 正式数据只保留当前唯一采用的游戏基线字段。不要在 JSON 里写来源强弱、wiki URL、
  推断过程、候选值、证据列表、考古备注、`sourceRank`、`candidate*`、`weak*`、
  `mobile*` / `fc2*` 对照字段或类似文档内容。
- 来源、置信度、被拒绝的候选、推断依据和争议说明写进 `work/*-card-*.md`、
  `work/recovered-data/` 或 `docs/reverse-archive/`；`reverse-notes.md` 只留当前结论和索引。
- 如果决定采用一个弱证据值，也要先在证据卡里说明弱点，然后把唯一采用值写成干净游戏数据。
  运行时代码不得靠“强来源/弱来源”字段选择分支。
- 修改 `server/data/*.json` 后必须跑 `node .\server\test-bootstrap-server.js`。该自检会拒绝
  正式数据里再次出现证据/来源/候选字段。
